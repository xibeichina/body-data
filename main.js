(function () {
  ...
  const metrics = [
    ...
    { key: "thigh_cm", label: "大腿围", unit: "cm" },
    { key: "arm_cm", label: "臂围", unit: "cm" }
  ];
  ...
  function render() {
    ...
    const weightDelta = previous ? latest.weight_jin - previous.weight_jin : 0;
    const deltaText = previous ? formatDelta(latest.weight_jin, previous.weight_jin) : "—";
    weeklyDeltaEl.innerHTML = `<span class="${weightDelta < 0 ? "delta down" : "delta"}">${deltaText}</span>`;
    ...
    const entryHTML = sorted.map((item, index) => {
      ...
      const chips = [
        ...
        item.thigh_cm ? `大腿围 ${item.thigh_cm} cm` : "",
        item.arm_cm ? `臂围 ${item.arm_cm} cm` : ""
  ...
  function renderChart(sorted) {
    ...
    const points = chronological
      .map(item => ({ date: item.date, value: Number(item[metric.key]) }))
    ...
    ctx.fillText(`${metric.label} (${metric.unit})`, padding.left, 18);
